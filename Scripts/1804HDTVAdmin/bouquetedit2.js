...
var imglist = [];
var imgremoved = [];
$("#imgPreview").find("[name='imgold[]']").each(function(i,data){
    //console.log($(this).find("#imgnum").val());
    imglist.push($(this).find("#imgnum").val());
});
...
$(document).on("click","#imgPreview div[name='imgshow[]']",function(){
...
$(document).on("click","#imgPreview div[name='imgold[]']",function(){
    var n = $(this).find("#imgnum").val();
    ...
    if (n) {
        ...
        imglist = imglist.filter(e=>e!=n);
        imgremoved.push(n);
        $("#imgPreview").append("<input type='hidden' name='imgremove[]' value='"+imgid+"'>");
    }
    console.log(imglist);
...
$("#frmEditBouquet").submit(function(event){
    ...
    if ($("#imgPreview").find("[name='img[]']").length || imgremoved.length>0) {
        imgchanged=true;
    }else{
        imgchanged=false;
    }
    ...
    if ($("#bid").val()== bid_old && $("#bname").val()==bname_old && $("#bprice").val()==bprice_old && $("#bdetail").val()==bdetail_old && bselling==bselling_old &&imgchanged==false) {
    // Nếu không thay đổi
